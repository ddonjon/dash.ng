...
export function Profile() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <button 
            onClick={() => navigate('/')}
            className="px-6 py-2 bg-purple-600 text-white rounded-xl font-medium"
          >
            Go Home
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-24 font-sans overflow-x-hidden">
      
      {/* Header */}
      <div className="sticky top-0 z-50 bg-[#F8F9FA]/90 backdrop-blur-md border-b border-gray-200">
      ...
      <div className="max-w-2xl mx-auto px-4 space-y-4 mt-4">
        ...
        <div className="bg-white rounded-[20px] border border-gray-200 shadow-sm overflow-hidden relative">
          ...
          <div className="p-4 relative z-10">
            {/* Compact Avatar & Info - Left Aligned */}
            ...
            <div className="grid grid-cols-3 gap-2">
              {/* Listings */}
              <div className="bg-gray-50/80 border border-gray-200 rounded-2xl p-2.5 flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-purple-100/60 flex items-center justify-center flex-shrink-0">
              ...
              <div className="bg-gray-50/80 border border-gray-200 rounded-2xl p-2.5 flex items-center gap-2">
                ...
                <div className="flex flex-col text-left">
                  ...
                  <span className="text-[9px] text-emerald-500 font-medium leading-tight">Items</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[20px] border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="p-3 space-y-3">
            ...
            <div className="w-full h-px bg-gray-200 ml-10"></div>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex-1">
                  ...
                  {isEditing ? (
                    <input
                      ...
                      className="border-b-2 border-purple-300 px-0 py-0.5 w-full md:w-3/4 focus:outline-none focus:border-purple-600 text-xs font-medium text-gray-900 bg-transparent transition-colors"
                      placeholder="+234..."
                    />
                  ) : (
              ...
              {!isEditing && (
                <button onClick={() => setIsEditing(true)} className="text-xs text-purple-600 font-semibold flex items-center gap-1 hover:text-purple-800">
                  {formData.whatsapp_number ? 'Edit' : 'Add'} <ChevronRight size={14} />
                </button>
              )}
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              {!isEditing && (
                <button onClick={() => setIsEditing(true)} className="text-xs text-purple-600 font-semibold flex items-center gap-1 hover:text-purple-800">
                  {formData.location ? 'Edit' : 'Add'} <ChevronRight size={14} />
                </button>
              )}
            </div>
          </div>
          ...
          {/* Edit Actions */}
          {isEditing && (
            <div className="flex gap-3 p-3 bg-gray-50/50 border-t border-gray-200">
              <button
                onClick={() => {
                  setIsEditing(false)
                  setFormData({
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 border-2 border-gray-300 rounded-xl text-xs font-bold text-gray-600 hover:bg-white hover:border-gray-400 transition-colors"
              >
                <X size={15} /> Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-xl text-xs font-bold hover:bg-purple-700 transition-colors disabled:opacity-70 shadow-sm"
              >
        ...
        {stats.totalListings === 0 && (
          <div className="bg-[#F8F5FF] rounded-[16px] p-3 border border-purple-200 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-bold text-purple-800">Ready to get started?</h3>
                <p className="text-[10px] text-purple-600/80 mt-0.5">Create your first listing.</p>
              </div>
            </div>
            <button
              onClick={() => navigate('/list-property')}
              className="flex items-center flex-shrink-0 px-3 py-1.5 bg-purple-600 text-white rounded-xl text-[10px] font-bold hover:bg-purple-700 transition shadow-sm"
            >
              + Add Listing
            </button>
          </div>
        ...
        <div className="bg-white rounded-[20px] border border-gray-200 shadow-sm overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between p-3 hover:bg-gray-50 transition border-b border-gray-200"
          >
